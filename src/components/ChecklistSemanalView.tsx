...
interface ChecklistSemanalViewProps {
  units: Unit[];
  tasks: StandardTask[];
...
export default function ChecklistSemanalView({
  units,
  tasks,
  executions,
  toggleTaskStatus,
  ...
  const weeklyTasks = useMemo(() => {
    return tasks.filter(t => t.frequency === 'semanal');
  }, [tasks]);
  ...
  }, [weeklyTasks, searchTerm]);

  // Helper to calculate weekly date range (Monday to Sunday containing selectedDate)
  const weekRange = useMemo(() => {
    const parts = selectedDate.split('-');
    if (parts.length < 3) return { label: 'Semana Operacional', dates: [] };
    ...
    const firstStr = `${dates[0].split('-')[2]}/${dates[0].split('-')[1]}`;
    const lastStr = `${dates[6].split('-')[2]}/${dates[6].split('-')[1]}`;
    return {
      label: `Semana de ${firstStr} a ${lastStr}`,
      dates
    };
  }, [selectedDate]);

  // Completion calculation (any execution in this week that is 'executado')
  const stats = useMemo(() => {
    ...
    weeklyTasks.forEach(task => {
      const isCompleted = executions.some(
        e => e.unitId === selectedUnitId && weekRange.dates.includes(e.date) && e.taskId === task.id && e.status === 'executado'
      );
  ...
  }, [selectedUnitId, weeklyTasks, executions, weekRange]);
  ...
  const handleOpenAddPendency = (task: StandardTask) => {
    ...
    setPendencyDesc(`Falha no Checklist Semanal [Unidade: ${currentUnit?.name}] - ${task.title} não realizado.`);
  ...
  const submitLinkedPendency = (event: React.FormEvent, taskId: string) => {
    ...
    onAddPendenciaObj({
      ...
      status: 'aberta',
      notes: `Registrado a partir do Checklist Semanal de Metas (Dia de check representativo: ${selectedDate})`
    });
  ...
  const getStatusStyle = (status: TaskStatus) => {
    switch (status) {
      ...
      default:
        return {
          icon: <Clock className="w-5 h-5 text-violet-400 font-bold" />,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-[#141414] p-5 rounded-xl border border-[#212121] shadow-md shadow-black/15 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <span className="text-xs font-mono text-gray-500 uppercase tracking-widest block font-bold mb-1">Rotinas de Média Frequência</span>
          <h2 className="text-2xl font-display font-extrabold text-white flex items-center gap-2">
            <CalendarDays className="w-6 h-6 text-purple-400" /> Checklist Semanal de Atividades
          </h2>
          <p className="text-xs text-gray-400 mt-1">
            Planejamento e aprovação de cronogramas semanais, postagem no TikTok e cobrança de vídeos locais para o Grupo ONE.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 px-3 py-2 bg-[#1A1A1A] border border-[#262626] rounded-xl text-xs font-mono text-gray-300 shrink-0">
          ...
          <span>Foco: <strong>{weekRange.label}</strong></span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-5 bg-[#141414] p-5 rounded-xl border border-[#212121]">
        <div className="md:col-span-4 space-y-2">
          <label className="block text-xs font-bold font-mono uppercase text-gray-450 tracking-wider">Unidade de Foco</label>
          <select
          ...
          {currentUnit?.contactName && (
            <p className="text-xs text-gray-500 italic mt-1 font-medium">Gerente responsável: {currentUnit.contactName}</p>
          )}
        </div>

        <div className="md:col-span-8 bg-[#1A1A1A] border border-[#262626] p-4 rounded-xl flex flex-col justify-between">
          <div className="flex justify-between items-center text-xs">
            <span className="text-gray-405 font-bold uppercase tracking-wide">Aderência Semanal Estimada</span>
          ...
          <div className="flex justify-between items-center text-[10px] text-gray-500 font-mono mt-1 pt-1.5">
            <span>Completadas na semana: <strong>{stats.completed}</strong></span>
            <span>Tarefas Semanais Totais: <strong>{stats.total}</strong></span>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4">
        <div className="flex flex-col sm:flex-row justify-between items-center gap-3">
          <div className="w-full sm:max-w-md">
            <input
              type="text"
              placeholder="Buscar por nome ou descrição da tarefa semanal..."
              value={searchTerm}
          ...
          <span className="text-[11px] text-purple-300 flex items-center gap-1.5 font-mono">
            <Sparkles className="w-3.5 h-3.5" /> Semanas operacionais sem amarrar a dias específicos
          </span>
        </div>
        ...
        <div className="space-y-4">
          {filteredTasks.length === 0 ? (
            <div className="bg-[#141414] border border-[#212121] rounded-xl p-10 text-center text-gray-500">
              Nenhuma ação semanal cadastrada ou correspondente à busca.
            </div>
          ) : (
            filteredTasks.map(task => {
              // Get the execution for this task within this week
              const execution = executions.find(
                e => e.unitId === selectedUnitId && weekRange.dates.includes(e.date) && e.taskId === task.id
              ) || executions.find(
                    ...
                    <div className="flex gap-3">
                      ...
                      <div className="space-y-1">
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          <button
                            type="button"
                            title="Clique para alternar prioridade da tarefa semanal"
                            onClick={() => {
                      ...
                      {editingNotesId === task.id ? (
                        ...
                        <div className="flex-1 flex items-center justify-between">
                          <span className="italic text-gray-450 truncate max-w-[400px]">
                            {pNotes ? `"${pNotes}"` : 'Sem observações semanais escritas.'}
                          </span>
                          <button
